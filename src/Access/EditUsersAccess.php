...
/**
* @file
* Contains \Drupal\administerusersbyrole\Acess\EditUsersAccess
*/
...
/**
 * Access check for user edition routes.
 */

class EditUsersAccess implements AccessCheckInterface {

  /**
   * {@inheritdoc}
   */
  public function applies(Route $route) {
    return array_key_exists('_access_administerusersbyrole_edit_users', $route->getRequirements());
  }
  ...
  public function access(Route $route, Request $request, AccountInterface $account) {
    ...
    // If it's of an administrator, deny the access
    ...
    if(in_array(DRUPAL_AUTHENTICATED_RID, $user_requested_roles) and
      ...
      if(in_array('edit users with no custom roles', $account_permissions)) {
        return AccessResult::allowed();
      }
    ...
    foreach ($user_requested_roles as $user_requested_role) {
      if(in_array("edit users with role $user_requested_role", $account_permissions)) {
        $hasPermission = true;
        break;
      }
    }
